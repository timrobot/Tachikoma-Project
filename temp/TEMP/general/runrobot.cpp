...
{
	chili.update(cameraFrame, camframelock, bearpos, bearsize, bearfound);
}

void bearcam_detect(void) {
  //detectBear(cameraFrame, camframelock, bearpos, bearsize, bearfound);
}
...
int main()
{
	...
	thread bearcam_thread(bearcam_detect);
...
static vec garm({ 489, 523, 857, 408 });
static vec pose1({ 363, 514, 716, 408 });
static vec pose2({ 500, 573, 900, 408 });
static vec pose3({ 363, 514, 716, 733 });
static vec pose4({ 500, 573, 900, 733 });
...
void manual_input(void)
{
	...
	while (!stopsig)
	{
		...
		/*if (secdiff(starttime, currtime) > 0.05)
		{
			...
			if (keystates[SDLK_p]) { garm = pose1; }
			if (keystates[SDLK_l]) { garm = pose2; }
			if (keystates[SDLK_o]) { garm = pose3; }
			if (keystates[SDLK_k]) { garm = pose4; }
...
void robot_calcmotion(void)
{
	while (!stopsig)
	{
		...
		// demo action of the motion does not require calcmotion
  ...
  int substate = 0;
	...
	while (!stopsig)
	{
		...
		if (!bearfound) {
		  ...
		  tachikoma.set_wheels(0.0, 0.0, 0.0, 0.0);
		  //garm = pose4;
		  tachikoma.set_arm(garm(0), garm(1), garm(2), garm(3));
		} else {
		  //cout << "found tag!\n";
		  if (state == 0) {
			if (WIDTH < 27.0) {
			  ...
			  POS(0) *= neggy;
			  vec motion = (turn * sqrt((double)POS(0) / 240.0) * neggy) + forward;
			  //motion *= 0;
			  tachikoma.set_wheels(motion(0), motion(1), motion(2), motion(3));
			  garm = pose1;
			  tachikoma.set_arm(garm(0), garm(1), garm(2), garm(3));
			}
		  }
		  ...
		  if (state == 3) {
			...
			tachikoma.set_arm(garm(0), garm(1), garm(2)-50, garm(3));
			  ...
			  if (substate == 0) {
			    ...
			    POS2(0) *= neggy;
				vec motion = (turn * sqrt((double)POS2(0) / 24.0) * neggy) + forward;
				tachikoma.set_wheels(motion(0), motion(1), motion(2), motion(3));
			  }
			  if (substate == 1) {
				tachikoma.set_wheels(0, 0, 0, 0);
