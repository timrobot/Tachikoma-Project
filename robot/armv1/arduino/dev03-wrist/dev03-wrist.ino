...
void setmotors(int topv, int btmv, int gv) {
  ...
  gv = gv * 90 / 255;
  grabber.write(limit(gv + 90, 0, 180));
}
...
static int targetv[3];
static int prevv[3];
static int targetp[3];
...
void loop() {
  ...
  if (leg_vel_act) {
    ...
    targetv[2] = (targetp[2] - analogRead(A2)) * 2;
  }

  int deltav[3] = { limit(targetv[0] - prevv[0], -4, 4),
                    limit(targetv[1] - prevv[1], -4, 4),
                    limit(targetv[2] - prevv[2], -4, 4) };
  v[0] = limit(prevv[0] + deltav[0], -255, 255);
  v[1] = limit(prevv[1] + deltav[1], -255, 255);
  v[2] = limit(prevv[2] + deltav[2], -255, 255);
  setmotors(v[0], v[1], v[2]);
  prevv[0] = v[0];
  prevv[1] = v[1];
  prevv[2] = v[2];
